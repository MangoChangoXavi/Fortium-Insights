  ...
  return (
    <>
        ...
        <div className="mx-auto justify-between px-4 md:flex md:items-center md:px-20 lg:max-w-7xl">
          <div>
            <div className="flex items-center justify-between py-3 md:block md:py-5">
              <Link href="#" className="">
                <p className="text-[23px] font-normal not-italic leading-[normal] text-primary-500">
                  {companyName}
                </p>
              </Link>
              ...
              <div className="md:hidden">
                ...
                >
                  <MenuIcon className="h-5 w-5 stroke-primary-500 hover:stroke-secondary-500" />
                </button>
              </div>
            </div>
          </div>
                ...
                if (hasPermissions(item.permissions)) {
                  return (
                    <Link
                      key={index}
                      href={item.link}
                      className="animate-fade-down text-darkGray animate-delay-100 animate-duration-1000 animate-ease-in-out flex w-full  transform flex-row items-center space-x-3  rounded-full duration-300 ease-in-out hover:ml-4 hover:text-primary-500 "
                    >
                      {item.icon}
                      <div>{item.name}</div>
                    </Link>
      ...
      >
        ...
        >
          {/* {!open ? <LogoIcon className="h-6 w-6 stroke-primary-500" /> : null} */}
          <p className="w-fit p-2 text-[23px] font-normal not-italic leading-[normal] text-primary-500">
            {companyName}
          </p>
          {open ? (
            <LogoIcon className="mr-2 h-8 w-8  stroke-primary-500 " />
          ) : null}
        </button>
        <div
          ref={maxSidebarRef}
              ...
              if (hasPermissions(item.permissions)) {
                return (
                  <Link
                    key={index}
                    href={item.link}
                    className="flex w-full transform flex-row items-center gap-[8px] text-[#2C2C2C] duration-300 ease-in-out  hover:text-primary-500"
                  >
                    {item.icon}
        ...
        <div className="absolute bottom-0 mx-[24px] flex w-full flex-col">
          ...
          <div className="border-t-gray -mx-[24px] mt-6 flex w-full flex-row items-center gap-4 border-t p-4">
            ...
            <button>
              <MenuIcon className="stroke-darkGray ml-2 h-6 w-6 hover:stroke-primary-500" />
            </button>
          </div>
        </div>
        ...
        >
          <div className="flex flex-col items-center justify-center gap-6">
            ...
            >
              <DashboardIcon className="fill-darkGray stroke-darkGray  h-5 w-5  hover:fill-primary-500 hover:stroke-primary-500" />
            </Link>
            {items.map((item, index) => {
              if (hasPermissions(item.permissions)) {
                return (
                  <Link
                    key={index}
                    href={item.link}
                    className="text-darkGray flex w-full transform justify-end px-3 duration-300  ease-in-out  hover:text-primary-500"
                  >
                    {item.icon}
                  </Link>
                );
